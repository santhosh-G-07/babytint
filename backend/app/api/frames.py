...
def update_frame(
    ...
    # slot_count always derives from the merged slot_positions, so the two
    # can never drift regardless of which fields this particular update
    # touched.
    frame.slot_count = len(frame.slot_positions)
    if frame.offer_price is not None and frame.offer_price >= frame.price:
        raise HTTPException(status_code=400, detail="offer_price must be less than price.")
