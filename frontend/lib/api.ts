import { clearAdminToken, getAdminToken } from "@/lib/local-admin-auth";
import type {
...
// next.config.mjs already fails the production build outright if this is
// missing. This is a defense-in-depth runtime check for any code path that
// reaches here without going through that build (e.g. `next dev` started
// with NODE_ENV=production by mistake) -- silently falling back to
// localhost would make every visitor's browser try to hit their OWN
...
async function apiFetch<T>(
  ...
  const hadToken = requireAuth && Boolean(getAdminToken());
  if (requireAuth) {
  ...
  if (!response.ok) {
    ...
    // Only force a re-auth redirect when a token existed and the server
    // just rejected it (expired/revoked mid-session) -- a 401 with no token
    // present is the normal "not logged in yet" case handled by page-level
    // guards, and redirecting there too would bounce every anonymous
    // visitor to /login.
...
export async function logout() {
  return apiFetch<{ ok: boolean }>("/api/auth/logout", { method: "POST" }, true);
}
