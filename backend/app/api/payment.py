...
_WEBHOOK_UNVERIFIED_ALLOWED_ENVS = {"development", "dev", "local", "test", "testing"}
...
def verify_razorpay_payment(
    ...
    query = select(Order).options(joinedload(Order.items)).with_for_update()
...
async def razorpay_webhook(
    ...
    elif settings.app_env.strip().lower() not in _WEBHOOK_UNVERIFIED_ALLOWED_ENVS:
