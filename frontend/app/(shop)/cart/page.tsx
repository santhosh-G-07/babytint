...
export default function CartPage() {
  ...
  // The frame snapshot cached on each cart item can go stale if an admin
  // deactivates the frame after it was added, so re-check live availability
  // whenever the cart page is open rather than trusting the cached flag.
  const { data: liveActiveById } = useQuery({
    queryKey: ["cart-frame-availability", cartFrameIds],
    queryFn: async () => {
      const entries = await Promise.all(
        cartFrameIds.map(async (frameId) => {
          try {
            ...
            // Frame fetch failed (e.g. deleted) -- treat as unavailable.
  ...
  const isItemActive = (frameId: string, cachedActive: boolean) =>
    liveActiveById?.[frameId] ?? cachedActive;
  ...
  const hasUnavailableItems = cart.some((item) => !isItemActive(item.frame.id, item.frame.is_active));
          ...
          <div className="mt-6 space-y-5">
            ...
            <div className="space-y-4">
              {cart.map((item) => {
                ...
                const itemActive = isItemActive(item.frame.id, item.frame.is_active);
                return (
                <div
                  key={item.id}
                    ...
                    <div className="flex-1">
                      ...
                      {assisted ? (
                        <p className="mt-1 text-sm text-emerald-700 dark:text-emerald-300">
                          BabyTint will contact you to design this after order confirmation.
                        </p>
                      ) : null}
