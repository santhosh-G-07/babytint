...
class CheckoutItem(BaseModel):
    frame_id: uuid.UUID
    quantity: int = Field(default=1, ge=1, le=MAX_ITEM_QUANTITY)
    # Intentionally no client-supplied price field: checkout() always
    # recomputes the price server-side from the frame's DB row via
    # order_item_unit_price(), so there is nothing here for a client price
    # to feed into even if it were accepted.
    customization_data: dict[str, Any]
